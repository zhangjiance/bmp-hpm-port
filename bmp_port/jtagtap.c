...
HPM_IOC->PAD[IOC_PAD_PB12].PAD_CTL = IOC_PAD_PAD_CTL_SR_MASK | IOC_PAD_PAD_CTL_SPD_SET(3);
HPM_IOC->PAD[IOC_PAD_PB13].PAD_CTL = IOC_PAD_PAD_CTL_SR_MASK | IOC_PAD_PAD_CTL_SPD_SET(3);
}
...
HPM_IOC->PAD[IOC_PAD_PB12].PAD_CTL = IOC_PAD_PAD_CTL_SR_SET(1) | IOC_PAD_PAD_CTL_SPD_SET(3);
...
static void jtag_spi_sequence_inner(uint32_t nbits, const uint8_t *tdi, uint8_t *tdo)
{
...
if (tx_idx < nb_bytes && !(JTAG_SPI_BASE->STATUS & SPI_STATUS_TXFULL_MASK))
JTAG_SPI_BASE->DATA = tdi[tx_idx++];
if (tdo && rx_idx < nb_bytes && !(JTAG_SPI_BASE->STATUS & SPI_STATUS_RXEMPTY_MASK))
...
spi_set_transfer_mode(JTAG_SPI_BASE,
tdo ? spi_trans_write_read_together : spi_trans_write_only);
JTAG_SPI_BASE->CMD  = 0xFF;
...
static void jtagtap_tms_seq_spi(uint32_t tms_states, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tdi_tdo_seq_spi(
const uint8_t *data_in, uint8_t *data_out, bool final_tms, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tdi_seq_spi(const uint8_t *data_in, bool final_tms, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_cycle_spi(size_t clock_cycles) __attribute__((optimize(3)));

static void jtagtap_tms_seq_spi(uint32_t tms_states, const size_t clock_cycles)
{
size_t cycle = 0;
...
while (cycle + run < clock_cycles &&
       (bool)((tms_states >> (cycle + run)) & 1U) == tms &&
       run < 32U)
run++;
...
static void jtagtap_tdi_seq_spi(const uint8_t *const data_in, const bool final_tms, const size_t clock_cycles)
{
...
static void jtagtap_tms_seq_clk_delay(uint32_t tms_states, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tms_seq_no_delay(uint32_t tms_states, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tdi_tdo_seq_clk_delay(
const uint8_t *data_in, uint8_t *data_out, bool final_tms, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tdi_tdo_seq_no_delay(
const uint8_t *data_in, uint8_t *data_out, bool final_tms, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tdi_seq_clk_delay(const uint8_t *data_in, bool final_tms, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_tdi_seq_no_delay(const uint8_t *data_in, bool final_tms, size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_cycle_clk_delay(size_t clock_cycles) __attribute__((optimize(3)));
static void jtagtap_cycle_no_delay(size_t clock_cycles) __attribute__((optimize(3)));
