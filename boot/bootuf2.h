...
struct bootuf2_STATE
{
    ...
    uint8_t *const Mask;
    uint8_t Enable;
    bool UpgradeComplete;  /* Flag to indicate upgrade finished */
    bool AppErased;        /* Flag to track if app region has been erased */
    uint32_t checksum_received;  /* Checksum of received UF2 data */
    uint32_t checksum_flash;     /* Checksum of Flash data after write */
};
...
#define BOOTUF2_BLOCKSMAX (((CONFIG_BOOTUF2_FLASHMAX) / 476) + (((CONFIG_BOOTUF2_FLASHMAX) % 476) ? 1 : 0))
...
/* 
 * Note: Flash operations and app control moved to port layer:
 *   - boot_flash_port_write()
 *   - boot_flash_port_erase_app()
 *   - boot_flash_port_check_app_valid()
 *   - boot_port_jump_to_app()
 * See port/boot_flash_port.h and port/boot_port.h
 */
